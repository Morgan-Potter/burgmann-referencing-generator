...
function sortReferences() {
    ...
    references.sort(
        ...
        // This could be a potential cause of error as it is only sorted by the first letter. The authors last names may be different, however they are still sorted by the first name.
        // This is an easy fix, however I am unsure if this method is incorrect. Additionally there is no solution for when the author and the year is the same currently implemented.
        function (a, b) {
            if (a[1].corporate != '' & a[1].corporate != undefined) {
                a[1].auth_last["1"] = a[1].corporate
                a[1].auth_first["1"] = a[1].corporate // if the corporate first
            }
...
function editReference() {
    let reference = localStorage.getItem("edit") // returns the reference id for edit e.g journal_1
    localStorage.removeItem("edit") // done so creating a new reference does not start this editing function
...
function addAuthor() {
    ...
    button.insertAdjacentHTML("beforebegin", '<input class="author" type="text" placeholder="Insert Author First Name"name="' + localStorage.getItem('auth_num') + ':auth_first" id="' + localStorage.getItem('auth_num') + ':auth_first"> <input class="author" type="text" placeholder="Insert Author Last Name" name="' + localStorage.getItem('auth_num') + ':auth_last" id="' + localStorage.getItem('auth_num') + ':auth_last"><br id="' + localStorage.getItem('auth_num') + ':br">');
